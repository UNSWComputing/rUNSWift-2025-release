...
if ! HOSTNAME=$( grep -Po "(?<=name = )[\w\-]+(?=;.*$(cat /sys/qi/head_id))" /home/nao/data/robots.cfg ); then
    exit 0
fi

if ! LAN=$( grep -Po "(?<=$(cat /sys/qi/head_id)).*" /home/nao/data/robots.cfg | grep -Po "(?<= lan = )[\d\.]+" ); then
    exit 0
fi

if ! WLAN=$( grep -Po "(?<=$(cat /sys/qi/head_id)).*" /home/nao/data/robots.cfg | grep -Po "(?<= wlan = )[\d\.]+" ); then
    exit 0
fi
...
cp "../../robots/robots.cfg" ./root/nao/data
chown 1001:1001 ./root/nao/data/robots.cfg
