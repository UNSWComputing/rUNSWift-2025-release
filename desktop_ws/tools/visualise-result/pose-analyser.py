...
def plotField(x_lists, y_lists, filenames):
    ...
    for i in range(len(x_lists)):
        plt.scatter(x_lists[i], y_lists[i], label=filenames[i], c='r', s=1)
...
def readXYs(filepath):
    ...
    for line in f:
        ...
        for i in range(nhyp):
            time_list.append( float(split[0]))
            x_list.append(    float(split[2 + i * 3]))
            y_list.append(    float(split[2 + i * 3 + 1]))
            theta_list.append(float(split[2 + i * 3 + 2]))
