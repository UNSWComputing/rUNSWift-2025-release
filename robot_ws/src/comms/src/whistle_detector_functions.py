    ...
    class SoundReceiverModule(SoundReceiverMixin):
        ...
        def listen_forever(self):
            ...
            while True:
                ...
                if self.state.whistleDone and (
                    self.node.previous_timestamp is None or self.node.previous_timestamp < time_stamp
                ):
                    self.publish(self.node.publisher, time_stamp)
                    self.node.previous_timestamp = time_stamp
                if count % 30 == 0:
                    rclpy.spin_once(self.node, timeout_sec=0.1)
                    if self.node.latest_game_state not in (CommsRCGCD.STATE_SET, CommsRCGCD.STATE_PLAYING):
                        return True

        def publish(self, publisher, timestamp):
            # Create and publish the message
            whistle_msg = CommsWhistle()
            whistle_msg.timestamp = get_aldebaran_timestamp_ros2(
                timestamp
            )  # Timestamp for when the message is being sent
            publisher.publish(whistle_msg)
            print(f"Published whistle info at {whistle_msg.timestamp}")
...
def get_aldebaran_timestamp():
    ...
    t = time.time()
    seconds = int(t)
    u_seconds = int("{:0.6f}".format(t % 1)[2:])
    timestamp = (seconds, u_seconds)
    return timestamp


def get_aldebaran_timestamp_ros2(aldebaran_timestamp):
    seconds, microseconds = aldebaran_timestamp  # Unpack the tuple
    nanoseconds = microseconds * 1000  # Convert microseconds to nanoseconds

    # Create ROS2 Time object
    ros_time = rclpy.time.Time(seconds=seconds, nanoseconds=nanoseconds).to_msg()
    return ros_time
