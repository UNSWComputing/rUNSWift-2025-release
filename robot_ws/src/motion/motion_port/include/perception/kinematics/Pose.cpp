...
{
  Eigen::Vector2i cartesian = coord.toCartesian();
...
RRCoord Pose::imageToRobotRelative(Eigen::Vector2i p, int h) const
{
  Eigen::Vector2i rr_p = imageToRobotXY(p, h);

  RRCoord r;
  r.setDistance(hypotf(rr_p.y(), rr_p.x()));
  r.setHeading(atan2f(rr_p.y(), rr_p.x()));
...
{
  return imageToRobotRelative(Eigen::Vector2i(x, y), h);
}

Eigen::Vector2i Pose::imageToRobotXY(const Eigen::Vector2i & image, int h) const
{
...
Eigen::Vector2i Pose::robotToImageXY(Eigen::Vector2i robot, int h) const
{
  ...
  return Eigen::Vector2i(pixel(0, 0), pixel(1, 0));
}
