...
{
public:
  explicit Walk2014Generator(const rclcpp::Node::SharedPtr & node);
  ~Walk2014Generator();
...
private:
   ...
   */
  void makeForwardKickJoints(
    float kickLean, float kickStepH, float & footh, float & forward, float & side,
    float & kneePitch, float & shoulderRoll, float & anklePitch, float & ballX, float & ballY,
    MotionCommand * request,
    const SensorValues & sensors, float & gyroXOntoSupportFoot);
