...
Walk2014Generator::Walk2014Generator(const rclcpp::Node::SharedPtr & node)
: Generator(node), t(0.0f), weightHasShifted(false)
{
  initialise();
  RCLCPP_INFO(motion_node_->get_logger(), "Walk2014Generator constructed");
}

Walk2014Generator::~Walk2014Generator()
{
  RCLCPP_INFO(motion_node_->get_logger(), "Walk2014Generator destroyed");
}

void Walk2014Generator::initialise()
{
  RCLCPP_INFO(motion_node_->get_logger(), "Walk2014 initializing");
  dt = MOTION_DT;
  ...
  kickStableNumFrames = 3;
  kickStableAngleX = 5.0;
  kickExtraStableAngleX = 4.0;
  kickExtraStableGyroscopeX = 5.0;
  kickGyroscopeXOntoSupportFootThresh = 5.0;
  kickGyroscopeXAwayFromSupportFootThresh = 30.0;
...
JointValues Walk2014Generator::makeJoints(MakeJointsIn & makeJointsIn)
{
  ...
  BodyModel & bodyModel = makeJointsIn.bodyModel;
  float ballX = makeJointsIn.ballX;
  float ballY = makeJointsIn.ballY;
  ...
  if (walk2014Option == NONE) {
    ...
    hiph =
      MAX(
      sensors.joints.angles[LKneePitch],
      0) / KNEE_PITCH_RANGE * (WALK_HIP_HEIGHT - STAND_HIP_HEIGHT) + STAND_HIP_HEIGHT;
  }
  ...
  if (t == 0) {
    if (shouldEmergencyStep) {
      ...
      forward = (float) active.twist.linear.x / MM_PER_M;          // in meters
      left = (float) active.twist.linear.y / MM_PER_M;             // in meters
      turn = active.twist.angular.z;                             // in radians
...
//            }
    ...
    if ((walk2014Option == WALK || walk2014Option == READY) &&
  ...
  if (active.action_type == BodyCommand::KICK &&
    request->body_command.action_type != runswift_interfaces::msg::BodyCommand::KICK)
  {
  ...
  } else if (active.action_type == BodyCommand::KICK) {
    ...
    // can change its mind.
    if (lastKickTime < 4 * T) {
      request->body_command.action_type = runswift_interfaces::msg::BodyCommand::WALK;
      ...
      if (walk2014Option == WALK) {
        ...
        if (fabs(forwardL) < EPSILON && fabs(leftL) < EPSILON && fabs(turnRL) < TURN_EPSILON &&
          t == dt)
        {
          // Assuming already at t = 0 from active getting set to kick
          // Any new settings the first time walk2014Option==KICK go here

          prepKick(active.foot == BodyCommand::LEFT, bodyModel);
        }
  ...
  if (walk2014Option == STAND) {   // Place CoM over ankle and turn set power to motors
    ...
    if (lastForward == 0) {
      ...
      targetComOffset = COM_OFFSET_BACKWARDS +
        ((abs(lastForward) / MAX_FORWARD) * (COM_OFFSET_FORWARDS - COM_OFFSET_BACKWARDS));
    } else {
      targetComOffset = COM_OFFSET_BACKWARDS;
    }
  ...
  if (walk2014Option == WALK and nextFootSwitchT > 0) {
    ...
    if (active.foot == BodyCommand::LEFT) {
      ...
      makeForwardKickJoints(
        kickLeanMod, kickHeight, foothL, forwardL, leftL, kneePitchL, shoulderRollL,
        anklePitchL, ballX, ballY, request, sensors, gyroXOntoSupportFoot);
      ...
      float gyroXOntoSupportFoot = -sensors.sensors[InertialSensor_GyroscopeX];
      makeForwardKickJoints(
        kickLeanMod, kickHeight, foothR, forwardR, leftR, kneePitchR, shoulderRollR,
        anklePitchR, ballX, ballY, request, sensors, gyroXOntoSupportFoot);
      leftR *= -1;
    }
  ...
  if (supportFoothasChanged and walk2014Option == WALK) {
    ...
    if (walkState == NOT_WALKING) {                           // Start the walk
      ...
      RCLCPP_FATAL(
        motion_node_->get_logger(), "Should never get here: walkState error");
    }
  ...
  sagittalBalanceAdjustment = KpGyro * filteredGyroY +
    (KdGyro * (filteredGyroY - preErrorGyro) / dt);
  ...
  sagittalHipBalanceAdjustment =
    -(KpAngle * angleError + KiAngle * angleErrorSum +
    (KdAngle * (angleError - preErrorAngle) / dt));
  ...
  *odometry = *odometry +
    motionOdometry.updateOdometry(sensors, updateOdometry(bodyModel.isLeftPhase));
  request->body_command.twist.linear.x = (float) (forward * MM_PER_M);
  request->body_command.twist.linear.y = (float) (left * MM_PER_M);
  request->body_command.twist.angular.z = (float) turn;
  ...
  if (legXL > thigh + tibia) {
    std::cout << "(Walk2014Generator) cannot solve Inverse Kinematics for that foot position!" <<
      std::endl;
  }
  ...
  if (shouldEmergencyStep) {
    active.left_arm_behind = true;
    active.right_arm_behind = true;
  }

  if (active.left_arm_behind) {
    j.stiffnesses[Joints::LShoulderPitch] = 0.3;
...
float Walk2014Generator::clamp(const int min, float value, const int max) const
{
...
void Walk2014Generator::makeForwardKickJoints(
  float kickingLean, float kickStepH, float & footh, float & forwardDist, float & side,
  float & kneePitch,
  float & shoulderRoll, float & anklePitch, float & ballX, float & ballY,
  MotionCommand * request, const SensorValues & sensors,
  float & gyroXOntoSupportFoot)
{
  bool ballXYDebug = false;
  // Don't execute kick if we're not stable yet. Once we've achieved the balancing criteria for
  if (kickT == 0) {
    if (request->body_command.extra_stable_kick) {
      // Ensure we're extra stable if we've been requested to. This sacrifices execution time for stability

      if (std::fabs(sensors.sensors[InertialSensor_GyroscopeX]) <
        DEG2RAD(kickExtraStableGyroscopeX) &&
    ...
    if (stableCounter < kickStableNumFrames) {
      return;                                             // Return if we're not stable yet
    }
  }
  ...
  float kickPower = power;
  float dynamicKickPhase = kickPhase + throughPhase * (1 - kickPower);
  float dynamicThroughPhase = kickPhase + throughPhase - dynamicKickPhase;
  ...
  if (kickT < backPhase) {
    ...
    // Swing foot forward.
  } else if (kickT < (backPhase + dynamicKickPhase)) {
    // Use hip balance now
    hipBalance = true;
    if (kickT >= backPhase + swingDelayFactor * dynamicKickPhase) {
      forwardDist = interpolateSmooth(
        kickBackAmp, kickAmp,
        kickT - backPhase - swingDelayFactor * dynamicKickPhase, dynamicKickPhase);
      anklePitch = interpolateSmooth(
        anklePitchRetracted, anklePitchAmp,
        kickT - backPhase - swingDelayFactor * dynamicKickPhase, dynamicKickPhase);
    }
    if (holdAnkle == true) {
    ...
    kneePitch = squareSmooth(
      kneePitchAmp * 0.9, kneePitchAmpEnd, kickT - backPhase,
      dynamicKickPhase);
    // Hold...
  } else if (kickT < (backPhase + dynamicKickPhase + dynamicThroughPhase)) {
    // Keep hip balance
    hipBalance = true;
    forwardDist = kickAmp;
    side = sideAmp;
    ...
    if (holdAnkle == true) {
      anklePitch = interpolateSmooth(
        anklePitchRetracted, anklePitchAmpFinal,
        kickT - backPhase - dynamicKickPhase, dynamicThroughPhase);
    } else {
      anklePitch = interpolateSmooth(
        anklePitchAmp, anklePitchAmpFinal,
        kickT - backPhase - dynamicKickPhase, dynamicThroughPhase);
    }
    //     // Return foot. @ijnek: this step was taken out in an attempt to stabilise the kick
    // } else if (kickT < (backPhase + dynamicKickPhase + dynamicThroughPhase + endPhase)) {
    //     forwardDist = interpolateSmooth(lastKickForward, 0, kickT - backPhase - dynamicKickPhase - dynamicThroughPhase, endPhase);
    //     side = interpolateSmooth(lastSide, 0, kickT - backPhase - dynamicKickPhase - dynamicThroughPhase, endPhase);
    //     shoulderRoll = interpolateSmooth(lastShoulderRollAmp, 0, kickT - backPhase - dynamicKickPhase - dynamicThroughPhase, endPhase);
    //     kneePitch = interpolateSmooth(lastKneePitch, 0, kickT - backPhase - dynamicKickPhase - dynamicThroughPhase, endPhase);
    //     anklePitch = interpolateSmooth(lastAnklePitch, 0, kickT - backPhase - dynamicKickPhase - dynamicThroughPhase, endPhase);
    //
    ...
    shoulderRoll = lastShoulderRollAmp - shoulderRollAmp * parabolicStep(
      kickT - totalPhase,
      shiftEndPeriod / 8, 0);
    kneePitch = lastKneePitch - lastKneePitch * parabolicStep(
      kickT - totalPhase,
      shiftEndPeriod / 8, 0);
    anklePitch = lastAnklePitch - lastAnklePitch * parabolicStep(
      kickT - totalPhase,
      shiftEndPeriod / 6, 0);
  ...
  if (kickT < (backPhase + dynamicKickPhase + dynamicThroughPhase)) {
...
void Walk2014Generator::readParameters()
{
  kickLeanOffsetL = motion_node_->get_parameter("kick_lean_offset_l").as_double();
  kickLeanOffsetR = motion_node_->get_parameter("kick_lean_offset_r").as_double();
  kickLean = motion_node_->get_parameter("kick_lean").as_double();
}

void Walk2014Generator::reset()
{
  initialise();
  RCLCPP_INFO(motion_node_->get_logger(), "Walk2014 reset");
}
...
void Walk2014Generator::stop()
{
  // this does nothing
  stopping = true;
  RCLCPP_INFO(motion_node_->get_logger(), "Walk2014 stop");
}
...
XYZ_Coord Walk2014Generator::mf2b(
  ...
  float xf, float yf, float zf)
{
  ...
  result.x = thigh * (s2 * s3 - c2 * c3 * c4) + tibia *
...
Walk2014Generator::Hpr Walk2014Generator::hipAngles(
  float Hyp, float Hp, float Hr, float Kp,
  float Ap, float Ar, float xf, float yf,
  float zf, XYZ_Coord e)
{
  ...
  float s5 = sin(Ap);
  float s6 = sin(Ar - 1.0 / 2.0 * pi);
  float j11 = thigh * (c4 * s3 + c2 * c3 * s2) - tibia *
    (s1 * (s2 * s3 - c2 * c3 * c4) - c5 * (c4 * s3 + c2 * c3 * s2)) +
    xf *
    (c1 * (s1 * (c4 * s3 + c2 * c3 * s2) + c5 * (s2 * s3 - c2 * c3 * c4)) - s5 *
    (s1 * (s2 * s3 - c2 * c3 * c4) - c5 * (c4 * s3 + c2 * c3 * s2))) +
