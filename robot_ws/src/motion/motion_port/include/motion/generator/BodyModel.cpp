...
void BodyModel::update(const SensorValues & sensors)
{
  ...
  float temp = sensors.sensors[Sensors::LFoot_FSR_FrontLeft]; if (fsLfl < temp and fsLfl < 5.0) {
    fsLfl = temp;
  }
  temp = sensors.sensors[Sensors::LFoot_FSR_FrontLeft];       if (fsLfl < temp and fsLfl < 5.0) {
    fsLfl = temp;
  }
  temp = sensors.sensors[Sensors::LFoot_FSR_FrontRight];      if (fsLfr < temp and fsLfr < 5.0) {
    fsLfr = temp;
  }
  temp = sensors.sensors[Sensors::LFoot_FSR_RearLeft];        if (fsLrl < temp and fsLrl < 5.0) {
    fsLrl = temp;
  }
  temp = sensors.sensors[Sensors::LFoot_FSR_RearRight];       if (fsLrr < temp and fsLrr < 5.0) {
    fsLrr = temp;
  }
  temp = sensors.sensors[Sensors::RFoot_FSR_FrontLeft];       if (fsRfl < temp and fsRfl < 5.0) {
    fsRfl = temp;
  }
  temp = sensors.sensors[Sensors::RFoot_FSR_FrontRight];      if (fsRfr < temp and fsRfr < 5.0) {
    fsRfr = temp;
  }
  temp = sensors.sensors[Sensors::RFoot_FSR_RearLeft];        if (fsRrl < temp and fsRrl < 5.0) {
    fsRrl = temp;
  }
  temp = sensors.sensors[Sensors::RFoot_FSR_RearRight];       if (fsRrr < temp and fsRrr < 5.0) {
    fsRrr = temp;
  }
  lastZMPL = ZMPL;
  ZMPL = 0;
